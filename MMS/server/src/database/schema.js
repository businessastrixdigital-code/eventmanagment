...
// 14. WhatsApp Connections
export const whatsappConnections = sqliteTable('whatsapp_connections', {
  id: text('id').primaryKey(),
  functionId: text('functionId').notNull().references(() => couples.id, { onDelete: 'cascade' }),
  hostGroup: text('hostGroup').notNull().default('HOST_A'), // HOST_A | HOST_B
  phone: text('phone'),
  sessionId: text('sessionId').notNull(),
  status: text('status').notNull().default('Disconnected'), // Disconnected | Connecting | Connected | QR_Ready
  connectedAt: text('connectedAt'),
...
export const communicationLogs = sqliteTable('communication_logs', {
  id: text('id').primaryKey(),
  notificationId: text('notificationId').references(() => notifications.id, { onDelete: 'cascade' }),
  functionId: text('functionId').notNull().references(() => couples.id, { onDelete: 'cascade' }),
  guestId: text('guestId').references(() => guests.id, { onDelete: 'set null' }),
...
export const whatsappConnectionsRelations = relations(whatsappConnections, ({ one }) => ({
  couple: one(couples, { fields: [whatsappConnections.functionId], references: [couples.id] }),
}));

export const communicationLogsRelations = relations(communicationLogs, ({ one }) => ({
  couple: one(couples, { fields: [communicationLogs.functionId], references: [couples.id] }),
  notification: one(notifications, { fields: [communicationLogs.notificationId], references: [notifications.id] }),
  guest: one(guests, { fields: [communicationLogs.guestId], references: [guests.id] }),
}));
